...
class Site_Factors():
    ...
    z1000_in_m : float
        z1000 (basin depth) in meters
    PGA_in_g : float
        PGA in g
    lenient : bool
        Whether to ensure the given Vs30, z1000, and PGA values are within the
    ...
    #%%------------------------------------------------------------------------
    def __init__(self, Vs30_in_meter_per_sec, z1000_in_m, PGA_in_g,
                 lenient=False):
        ...
        status = Site_Factors._range_check(Vs30_in_meter_per_sec, z1000_in_m,
                                           PGA_in_g)
        if 1 in status:
            if not lenient:
        ...
        if 2 in status:
            if not lenient:
                raise ValueError('z1000_in_m should be between [8, 900] m')
            else:
                z1000_in_m = 8 if z1000_in_m < 8 else 1000
        if 3 in status:
            if not lenient:
        ...
        if 4 in status:
            raise ValueError('Vs30 and z1000 combination not valid')

        self.Vs30 = Vs30_in_meter_per_sec
        self.z1000 = z1000_in_m
        self.PGA = PGA_in_g
    ...
    def _get_results(self, amplif_or_phase, data_dir, method='nl_hh',
        ...
        points = []  # to hold reference (Vs30, z1000, PGA) points
        y_list = []  # to hold values at these reference points
        for Vs30_i, z1000_i, PGA_i in combinations:
        ...
        y_interp \
            = Site_Factors._interpolate(points, y_list, (Vs30, z1000, PGA))

        if show_interp_plots:
            Site_Factors._plot_interp(points, (Vs30, z1000, PGA),
    ...
    #%%------------------------------------------------------------------------
    @staticmethod
    def _query(amplif_or_phase, Vs30, z1000, PGA, Fourier=True,
               method='nl_hh', data_dir=None):
        '''
        Query amplification or phase factors from pre-computed .csv files. The
        given Vs30, z1000_in_m, and PGA_in_g values need to match the
        pre-defined values (see `Vs30_array`, `z1000_array`, and `PGA_array`
        ...
        if z1000 not in Site_Factors.z1000_array:
            raise ValueError('`z1000` should be in %s.' % Site_Factors.z1000_array)
        ...
        if amplif_or_phase == 'amplif':
            if Fourier:
                y_filename = '%d_%03d_af_fs_%s_avg.csv' % (Vs30, z1000, method)
                x_filename = '%d_%03d_freq.csv' % (Vs30, z1000)
            else:  # response spectra
                y_filename = '%d_%03d_af_rs_%s_avg.csv' % (Vs30, z1000, method)
                x_filename = '%d_%03d_period.csv' % (Vs30, z1000)
        else:  # phase shift
            y_filename = '%d_%03d_phase_shift_%s_avg.csv' % (Vs30, z1000, method)
            x_filename = '%d_%03d_freq.csv' % (Vs30, z1000)
    ...
    def _locate_grids(self):
        '''
        Locates the "reference grids", i.e., rereference Vs30, z1000, and PGA
        values (in terms of the indices, not actual values).

        Returns all possible combinations of Vs30, z1000, and PGA values.
        '''
        Vs30_loc, z1000_loc, PGA_loc \
            = Site_Factors._find_neighbors(self.Vs30, self.z1000, self.PGA)

        combinations = list(itertools.product(Vs30_loc, z1000_loc, PGA_loc))
    ...
    #%%------------------------------------------------------------------------
    @staticmethod
    def _find_neighbors(Vs30_in_mps, z1000_in_m, PGA_in_g):
        '''
        Find the indices of Vs30, z1000, and PGA that surround the provided values.
        If the provided values fall onto the "reference" Vs30, z1000, or PGA values,
        ...
        z1000_loc = Site_Factors._search_sorted(z1000_in_m, Site_Factors.z1000_array)
        PGA_loc = Site_Factors._search_sorted(PGA_in_g, Site_Factors.PGA_array)

        return Vs30_loc, z1000_loc, PGA_loc
    ...
    def _plot_interp(ref_points, query_point, T_or_freq, amps, amp_interp,
        ...
        reference (Vs30, z1000, PGA) points, as well as the interpolated factors.

        Parameters
        ----------
        ref_points : list of tuples
            List of tuples of (Vs30, z1000, PGA), which are the reference points
        query_point : tuple
            A tuple of (Vs30, z1000, PGA) at which you want to query the factors
    ...
    #%%------------------------------------------------------------------------
    @staticmethod
    def _range_check(Vs30_in_mps, z1000_in_m, PGA_in_g):
        '''
        Check if the provided Vs30, z1000_in_m, and PGA_in_g values are within
        the pre-computed range.

        Meanings of different return values:
        1 - Vs30 outside of range [175, 950] m/s
        2 - z1000_in_m outside of range [8, 900] m
        3 - PGA outside of range [0.01g, 1.5g]
        4 - Vs30 and z1000 combination is not valid
        '''

        if not isinstance(Vs30_in_mps, (float, int, np.number)):
            raise TypeError('Vs30 must be int, float, or numpy.number.')
        if not isinstance(z1000_in_m, (float, int, np.number)):
            raise TypeError('z1000_in_m must be int, float, or numpy.number.')
        ...
        if Vs30_in_mps < 175 or Vs30_in_mps > 950:
            status.append(1)
            return status
        if z1000_in_m < 8 or z1000_in_m > 900:
            status.append(2)
            return status
        if PGA_in_g < 0.01 or PGA_in_g > 1.5:
            status.append(3)

        if Vs30_in_mps > 400 and z1000_in_m > 750:
            status.append(4)
        elif Vs30_in_mps > 450 and z1000_in_m > 600:
            status.append(4)
        elif Vs30_in_mps > 550 and z1000_in_m > 450:
            status.append(4)
        elif Vs30_in_mps > 600 and z1000_in_m > 300:
            status.append(4)
        elif Vs30_in_mps > 650 and z1000_in_m > 150:
            status.append(4)
        elif Vs30_in_mps > 750 and z1000_in_m > 75:
            status.append(4)
        elif Vs30_in_mps > 800 and z1000_in_m > 36:
            status.append(4)
        elif Vs30_in_mps > 850 and z1000_in_m > 16:
            status.append(4)
        else:
            pass
